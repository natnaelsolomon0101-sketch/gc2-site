...
export default function Ticker() {
  return (
    <div aria-hidden className="overflow-hidden border-y border-stone bg-paper-white">
          ...
          <div className="flex shrink-0" key={set}>
            {TICKER.map((t) => (
              <div key={`${set}-${t.symbol}`} className="flex items-baseline gap-2.5 whitespace-nowrap px-7 py-3.5">
                <span className="text-[12px] font-medium tracking-[0.06em] text-graphite-ink">{t.symbol}</span>
                <span className="text-[11px] uppercase tracking-[0.12em] text-pebble">{t.label}</span>
              </div>
            ))}
          </div>
        ))}
