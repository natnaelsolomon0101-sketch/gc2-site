...
/* Local Editorial Hero header (21st Editorial Hero 19075: tagline left,
   ...
   structure rather than forking `PageHeader` itself. Reused verbatim by
   /diligence, /questions, /access and /letters — same file shape in each,
   never imported across those directories, so no route depends on another
   route's internals. `emphasize` finds the FIRST whole-word match of `word`
   in `title` and wraps it in <em>; a miss degrades to the plain title rather
   than throwing, the same fallback ForAllocators.tsx's copy of the helper
   uses. */
