...
import { fetchCnbc, fetchTape, fetchRubenstein, fmtWhen } from "@/lib/news";
...
export default async function MarketsPage() {
  const [quotes, cnbc, tape, ruben] = await Promise.all([
    fetchQuotes(), fetchCnbc(), fetchTape(), fetchRubenstein(),
  ]);
  ...
  return (
    <>
      ...
      {(watching.length > 0 || ruben.length > 0) && (
        <section className="mk-sec" aria-labelledby="mk-watch">
          <div className="wrap">
              ...
              {watching.map((v) => (
                <li key={v.id}>
                  <Glass as="div" radius={18} className="mk-vid">
                    <div className="mk-vid-media"><LiteVideo id={v.id} title={v.title} /></div>
            ...
            {ruben.length > 0 && (
              <div className="mk-col" style={{ marginTop: 36 }}>
                <h3 className="t-caption">David Rubenstein · in the news</h3>
                <ul className="mk-list">
                  {ruben.map((h) => (
                    <li key={h.url}>
                      <Glass as="div" radius={14}>
